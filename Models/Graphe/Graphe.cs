...
using Transconnect.Algorythme.Parcours;
...
namespace Transconnect.Models.Graphe
{
    public class Graphe
    {
        ...
        public bool EstConnexe()
        {
            ...
            ParcoursLargeur parcoursLargeur = new ParcoursLargeur(this);
            var noeudsVisites = parcoursLargeur.Parcourir(Noeuds[0]);

            return noeudsVisites.Count == Noeuds.Count;
        }
        public bool ContientCycles()
        {
            HashSet<Noeud> visites = new HashSet<Noeud>();
            HashSet<Noeud> enRecursion = new HashSet<Noeud>();

            foreach (var noeud in Noeuds)
            {
                if (!visites.Contains(noeud))
                {
                    if (DetecterCycleDFS(noeud, visites, enRecursion))
                        return true;
                }
            }
            return false;
        ...
        private bool DetecterCycleDFS(Noeud noeud, HashSet<Noeud> visites, HashSet<Noeud> enRecursion)
        {
            visites.Add(noeud);
            enRecursion.Add(noeud);

            foreach (var lien in Liens)
            {
                if (lien.Noeud1 == noeud)
                {
                    var voisin = lien.Noeud2;

                    if (!visites.Contains(voisin))
                    {
                        if (DetecterCycleDFS(voisin, visites, enRecursion))
                            return true;
                    }
                    else if (enRecursion.Contains(voisin))
                    {
                        return true;
                    }
                }
